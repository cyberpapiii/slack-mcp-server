...
func registerCustomPowerTools(s *mcpserver.MCPServer, api *provider.ApiProvider, logger *zap.Logger, enabled []string, approvals *approval.Store) {
	if service, err := api.MessageFiles(); err == nil {
		...
		add(ToolFilesUpload, "Upload a file to Slack, and post it as a message when shared into a channel. This is also how you send a message with an attachment: pass channel_id plus initial_comment and the file and text post together as one message. Uses Slack's supported external-upload flow.", []mcp.ToolOption{
