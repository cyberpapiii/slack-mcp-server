...
func registerDailyPowerLifecycleTools(s *mcpserver.MCPServer, api *provider.ApiProvider, logger *zap.Logger, enabledTools []string) {
	...
	if dnd, err := api.DND(); err == nil {
		...
		addEnabledTool(s, enabledTools, newDailyPowerTool(ToolDNDEndSnooze, mcp.WithDescription("End the authenticated user's current Do Not Disturb snooze.")), h.EndSnooze)
...
func registerChannelMutationTools(s *mcpserver.MCPServer, h *handler.ChannelMutationHandler, enabledTools []string) {
	addEnabledTool(s, enabledTools, newDailyPowerTool(ToolChannelsRename, mcp.WithDescription("Rename one ordinary channel."), mcp.WithString("channel_id", mcp.Required(), mcp.Description(descChannelIDRaw)), mcp.WithString("name", mcp.Required(), mcp.Description("New channel name, 1 to 80 lowercase letters, numbers, hyphens, or underscores."))), h.ConversationsRenameHandler)

	addEnabledTool(s, enabledTools, newDailyPowerTool(ToolChannelsSetTopic, mcp.WithDescription("Set or clear one ordinary channel topic."), mcp.WithString("channel_id", mcp.Required(), mcp.Description(descChannelIDRaw)), mcp.WithString("topic", mcp.Required(), mcp.Description("New topic, up to 250 characters; pass an empty string to clear it."))), h.ConversationsSetTopicHandler)

	addEnabledTool(s, enabledTools, newDailyPowerTool(ToolChannelsSetPurpose, mcp.WithDescription("Set or clear one ordinary channel purpose."), mcp.WithString("channel_id", mcp.Required(), mcp.Description(descChannelIDRaw)), mcp.WithString("purpose", mcp.Required(), mcp.Description("New purpose, up to 250 characters; pass an empty string to clear it."))), h.ConversationsSetPurposeHandler)
